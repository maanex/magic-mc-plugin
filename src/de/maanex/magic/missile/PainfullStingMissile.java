...
public class PainfullStingMissile extends MagicMissile {
	...
	public void tick() {
		if (fuel-- <= 0 || target == null || target.getLocation().distance(position) <= 1) {
			...
			destroy();
			ParticleUtil.spawn(Particle.BLOCK_CRACK, position, 300, .2, 2, 2, 2, Material.DIAMOND_BLOCK.createBlockData());
			return;
		}
		...
		ParticleUtil.spawn(Particle.BLOCK_CRACK, position, 10, .2, 0, 0, 0, Material.DIAMOND_BLOCK.createBlockData());
